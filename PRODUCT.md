...
- Always display studio logos and marks straight on, preserving their original proportions. Never rotate or slant them, including during animation. Do not append captions, subtext, or extra taglines to studio branding; the full logo already includes its tagline. The owner restored the Home atmospheric animation around the static mark; preserve the mark itself without rotation, recoloring, or distortion.
...
- Home: studio-first hero, primary Meet the team linking to /about#team, secondary Meet Trash Dash linking to /games, woodland first-game feature, DIY studio story, social/email contact. No team portraits on Home. The approved transparent Trashy idle animation loops silently in the game feature while it is in view, pauses offscreen/when hidden, and keeps a still fallback for reduced motion, errors, or unsupported transparency. Safari always uses the still with a gentle CSS idle motion. No playback button or duplicate CTA overlays the artwork. The Home hero uses the static upright studio mark on navy (#091e2a), enlarged to fill the space beside the text, with cursor-responsive mist and fireflies spanning the entire hero behind the artwork and copy. The effects settle when idle and stay static for reduced motion. The intro video is no longer used.
...
- Characters: a focused overview of Trashy and Jimothy plus a separate Meet the villains section for the Chapter 1 Trash Monster and Chapter 2 Brutus. Profiles use owner-supplied artwork and source-confirmed canon: both bosses are Gunk-infected victims, and Brutus still recognizes Trashy.
- Media: filterable artwork/logo gallery, an in-page viewing-only preview, and data-driven support for captioned self-hosted video. The first video is the 30-second "How it started" montage, shown with the first character sketches and James's origin story. There is no public upload form or CMS.
- About Us: the supplied yellow-background studio intro plays once muted in the hero and holds its final frame. Preserve its full 16:9 frame with no added effects. The video gets the larger desktop column, with the text-to-video gap equal to the right outer inset; on mobile it spans the content width. Reduced motion and playback failures show the final-frame still. Includes the homegrown studio origin story and the four approved family members, exact bios and roles, owner-supplied portraits. The #team anchor is the primary homepage destination; existing person anchors remain.
